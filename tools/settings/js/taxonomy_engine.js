...
/**
 * Standalone Taxonomy Manager. Build 5.2.6
 * Role: Handles UI rendering, state for Genre/Sub-Genre management, and Drawer logic.
 * Build 5.2.6: Accessibility Hardening (Emoji Scrub) and Persistence Sync.
 */
...
window.metaforge.settings.taxonomy = {
    ...
    /**
     * Persists the taxonomy matrix to taxonomy.json.
     * Implements Surgical Emoji Scrub for AT status.
     */
    save: async function() {
        ...
        try {
            ...
            if (r.status === 'success') {
                ...
                status.innerHTML = "<span style='color:var(--status-success);'><span aria-hidden='true'>✓</span> Taxonomy matrix physically committed.</span>";
